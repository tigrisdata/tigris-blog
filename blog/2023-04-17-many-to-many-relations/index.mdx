...
description: >
  This blog post shows how to model many-to-many relations using embedded documents and referencing another collection. 
...
post and we will dive into modeling many-to-many relations. We model many-to-many relations in two ways. As embedded documents
...
## Embedded Many-to-Many

Let us start with modeling one-to-many relations with an embedded document. To do that we will model a set of courses and students.
...
In that file create a course collection that will look like the below:
...
We have created two models, each in their own collection. The first collection, `course`, contains all the courses.
The second collection is the `student` collection. In this collection we embedd which courses the student is attending.
...
Now lets change the `index.ts` file so that we can explore this design:
...
Lines 5 to 10 we setup our database and collections. On line 19 we start a transaction before we add
...
that links a student to a course. Lets do this by editing our `courses.ts` file to look like this:
...
This forces a unique constraint so that a `student` will only be signed up to a course once.

Lets add data and explore this model. Change your `index.ts` to look like this:
