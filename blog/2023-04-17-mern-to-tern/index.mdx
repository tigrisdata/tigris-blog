...
once was, it's still a viable and productive choice with more flexibility than
...
In this post, you'll learn why you should migrate from MERN to TERN. We'll cover
all the steps involved in migrating a MERN application to **TERN** (**T**igris,
**E**xpress.js, **R**eact, and **N**ode.js); first using
...
wire protocol support), remove the unused mongoose dependency, update the code
to use Promises, define the database name in an environment variable, use
...
The `@TigrisCollection("records")` decoror and supplied `records` value informs
Tigris that there is a `records` collection of documents of type `Record` in the
structure defined by that class.
...
With the schema defined, we can move on to updating application code.
...
displaying the employees but via the Tigris SDK.
...
for the TigrisSDK differs from the MongoDB Node.js driver in that, it takes a
...
Let's add the Tigris logo 😄 Update `mern/client/src/components/navbar.js` as
follows:
...
Since there are no changes to the front-end, stop and start the server:
...
If you have a look at the Tigris Console and explore the data, as shown in the
video, you'll see the `_id` values are structured differently, and the `$k`
