...
This is part 2 of the **Tigris Data Modeling Series** and we explore modeling one-to-many relations.
We look two ways to implement this. The first is using an embedded pattern and then using a separate collection with a relation field. We will also look at when it is best to use either of these designs.

![Modeling one-to-many image](modeling-one-to-many.png)

<!--truncate-->

If you missed the first in the series, head over to [modeling one-to-one relations](../2023-03-02-one-to-one-relations)
...
Let us start with modeling one-to-many relations with an embedded document. In the playground repository that we
set up, there is a post model. It can be found at `src/db/models/post.ts`.
...
In the above code, we created a new `Comment` class on line 8. We then embedd this as an array in our `Post` object on `line 39`.
Now every time there is a new comment we will fetch the post document from Tigris and append the latest comment.
...
The main lines to focus on are `line 18` to `line 43`. Here we create a post and save it to Tigris. Then we add to the `comments` field
in the document and save it to the server.
Finally, we find the first document in the collection that matches the title and we print it to the console. This is how you can work with one-to-many relations using the embedded pattern.
...
However, if there are a lot of writes, we will get a lot of conflicts. We can handle this by either retrying the update or fetching the document
...
collections. We will query from the blog collection to get the blog post we want and then from the comments collection to get all the comments
...
On `line 2` we import the new collection and add it to our database schema on `line 10`. The main query changes start at `line 33`,
where we insert each comment individually. In this small example, we could use `insertMany` to
batch insert the comments, but I've intentionally inserted them individually as if they were inserted by separate requests from web requests.
On `line 47` we start a transaction so that we can read from the post collection and the comment collection. We create a transaction so that we
...
The two different one-to-many relation design patterns here work well when used correctly. If you are not doing a lot of updates and not adding a lot of documents related to the parent then
the embedded pattern will be the most efficient. However, if there are a lot of documents being linked to the parent document, then a separate collection will be the best pattern to follow.
