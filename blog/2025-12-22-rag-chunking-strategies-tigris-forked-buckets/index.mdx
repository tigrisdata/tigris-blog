...
> Note: To run [The Best Chunking Method](https://github.com/davidmyriel/tigris-rag-lab), you will need an API keys from [Tigris Data](https://www.tigrisdata.com), [Qdrant Vector Database](https://qdrant.tech) and [OpenAI](https://openai.com/index/openai-api/) for generation. We also use [CrewAI](https://www.crewai.com) to orchestrate agent operations.

![Image 0](./image0.png)

### **Why forking matters for dataset quality**
...
In this app, [CrewAI](https://www.crewai.com/) is easy because it turns the whole experiment loop into one clear workflow instead of a pile of glue code. The [Streamlit](https://streamlit.io) UI collects the input, then CrewAI runs the same sequence for each chunking strategy:   
...
To use Qdrant Cloud from this app, you'll need an API key. You can create one in the Qdrant Cloud dashboard by following the [Qdrant Cloud API key guide](https://qdrant.tech/documentation/cloud/quick-start/#create-an-api-key). If you prefer a managed control plane for your agents, you can run this workflow on [CrewAI Cloud](https://www.crewai.com/cloud), which handles orchestration and scaling for you.
...
4. computes metrics across the ranked results:  
* **Hit@k**: did *any* of the top-k chunks overlap a gold span?  
* **MRR@k**: what's the rank of the *first* overlapping chunk?  
* **Coverage@k**: what fraction of the gold span(s) are covered by the retrieved chunks?  
* **Redundancy@k**: how much overlap exists *between returned chunks* (wasted retrieval budget)?
...
If you're serious about document chunking, check out the **Tigris RAG Lab** …and you'll never have to wonder which dataset variant your "best score" came from again.

### **Annex: App Architecture Overview**

[Tigris RAG Lab is a Streamlit web app](https://github.com/davidmyriel/tigris-rag-lab) that automates RAG chunking experiments using a three-tier architecture: 

- Tigris S3 (dataset versioning via buckets, snapshots, and forks),  
- Qdrant (vector indexing with per-strategy collections),   
- CrewAI (orchestrating experiments). 
