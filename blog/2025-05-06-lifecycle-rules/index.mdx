...
implement. Especially when you’re building for multicloud across many regions.
Let’s walk through how we built the feature, and where things got a little
complicated.
...
Most of the hard work we put into this was separating out the concepts of “last
time the object bytes were modified” and “last time the object metadata was
modified”. This is a pretty scary thing to do and required _a lot_ of test code,
manual testing, and testing in production. We have a number of internal test
buckets we use to keep track of Tigris’ performance, and the most scary part of
this change was enabling the separation of timestamps logic on those buckets.
Nothing went wrong, thankfully, but it’s always scary to hit the big red button
and know that you’re probably about to get paged if something goes wrong.
