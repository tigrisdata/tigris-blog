...
set recording bot for [PonyvilleFM](https://ponyvillefm.com/). I've been
involved with this station for over a decade and I met my husband in its
community. I've been running their recording bot since the transition from IRC
to Discord and I've accumulated over 600 GB of DJ sets, including ones that were
thought to be lost media.

Needless to say, this dataset is very important to me. I want to make sure that
it's backed up and available for anyone that wants to go back and listen to the
older sets. I also don't want to just dump it on the internet archive or
something like that, I want them to be easily accessible for anyone that wants
them.
...
persistent storage. In my homelab cluster, I have two
[StorageClasses](https://kubernetes.io/docs/concepts/storage/storage-classes/):
NFS mounting folders from my NAS (via the
[nfs-subdir-external-provisioner](https://github.com/kubernetes-sigs/nfs-subdir-external-provisioner)),
and [Longhorn](https://longhorn.io).

One of the main reasons why I use Longhorn for most of the storage in my homelab
is that it natively supports two things that make it possible for me to reboot
machines at will without any perceived downtime:

- All volumes get replicated to (at least) three machines in the homelab
- All volumes get backed up to [Tigris](https://tigrisdata.com) automatically at
  midnight

The main reason I can't use Longhorn for this is that I'd want the volume to be
at least a terabyte, and putting it in Longhorn would mean I'd need three copies
of that data (I could edit the StorageClass definition to avoid this or add
rotational drives to the cluster so that it wouldn't be as much of an issue, but
I didn't have free drives at the time). My homelab machines only have 1TB SSDs
each. Add in that the data is infrequently accessed (so it being on an SSD is
overkill) and it would basically fill up my entire homelab's storage budget, I
don't think that it's a good idea for this workload.

I could also put that data into my NAS via NFS, but I'd really feel better if
that data was automatically backed up to the cloud. I could configure something
to do that with a CronJob and a Tigris access key, but that would require extra
configuration. I'd really rather it be done by default or without extra
...
assuming that it's writing directly to a filesystem. I don't want to change this
too much, this bot is annoying to test and it's been stable for a very long time
as-is.
...
Sure in theory having a Tigris bucket as a filesystem would mean that there's
more latency for reading and writing files, but this data is infrequently
accessed. It's okay for it to be slower and people have no expectations for
super-fast downloads of DJ sets from years ago. If it ever became an issue, I
could change the logic to forward people to a pre-signed download URL instead of
the file directly in storage.

I did some digging and found out about Yandex's
[csi-s3](https://github.com/yandex-cloud/k8s-csi-s3). It's a StorageClass
implementation that uses S3 buckets as its backing store. Unlike a lot of other
StorageClass implementations I've tried this year, csi-s3 was really really easy
to install. All I had to do was apply the release with
...
out of rclone (or if I copied the data to my NAS with its 2.5 gigabit NIC), but
I started it, went to sleep, and when I woke up it was done. When I looked back
over the logs, I noticed that the main reason why it took so long was that a lot
of the older files were many small files. Tigris handles many small files
...
## It works thanks to Tigris

I don't have to worry about the DJ set server running out of space anymore. I
can just sit back and let it do its thing, all thanks to Tigris taking away the
hard parts from me.

It just works, and that's such a relief. If you’re looking for a lightweight way
to manage persistent storage in Kubernetes without a ton of overhead, I’d
