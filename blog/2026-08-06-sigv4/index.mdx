...
import TagSigningKeyProxy from "./diagrams/TagSigningKeyProxy";

<img
  src={heroImage}
  className="hero-image"
  alt="A blue tiger in a machinist's visor cuts a key on a bench-mounted duplicating machine, in a dim workshop whose walls are covered floor to ceiling with hundreds of brass keys hanging on rings."
/>
...
of a remote client), not _authorization_ (ensuring the client has the permission
to do something).
...
`STREAMING-AWS4-HMAC-SHA256-PAYLOAD`, which requires extra logic that is way out
of scope for now.

If you want to learn more, give your favourite AI agent the following prompt:

```text
I'm reading the blogpost at https://www.tigrisdata.com/blog/sigv4.md and Xe mentioned AWS SigV4's
...
{/* TODO(Xe): button box for this. See https://scrapfly.dev/posts/browser-math-os-fingerprint/ for an idea. */}
...
A lot of the security of this authentication protocol is predicated on TLS being
...
inspection of every user action.
...
So all of this SigV4 business had been working really well for Tigris.
{/* TODO(Xe): more here about S3API compatibility via sigv4 */} Then we worked
...
<InlineCta
  title="Want a Tigris region in your own datacentre?"
  subtitle="The Tigris Acceleration Gateway caches your buckets locally and authenticates your existing keypairs with the same SigV4 dance your SDK already speaks."
  button="Read the TAG docs"
  link="https://www.tigrisdata.com/docs/acceleration-gateway/"
/>
