...
| 🏗️ **Approach**            | 💾 **Storage Format**       | 🚀 **Streaming**         | 🔄 **DDP Compatible**       | ⚙️ **Setup Complexity** |
| -------------------------- | --------------------------- | ------------------------ | --------------------------- | ----------------------- |
| **Custom IterableDataset** | Raw files + manual sharding | ✅ Manual implementation | ✅ Manual sharding required | 🔴 High                 |
| **WebDataset**             | `.tar` shards over HTTP/S3  | ✅ Built-in streaming    | ✅ Automatic                | 🟡 Medium               |
| **HuggingFace Datasets**   | Various formats on HF Hub   | ✅ Streaming mode        | ⚠️ Partial support          | 🟢 Low                  |
| **LanceDB + Tigris**       | Optimized columnar format   | ✅ Native streaming      | ✅ Built-in sharding        | 🟢 Low                  |
...
object storage backend that puts your files close to where they’re being used.
...
so you don’t need to manage your data separately from your database.
...
You can also pass credentials directly to LanceDB in your code. LanceDB’s
...
Now, let’s connect LanceDB to your Tigris bucket. In Python, use the
...
Here’s a code snippet that converts the Food 101 dataset into a LanceDB table
stored in Tigris:
...
def create_lance_dataset(dataset, output_uri, fragment_size=12500, batch_size=1024):
    ...
    print(f"✅ Lance dataset written to {output_uri}")
...
- The `create_lance_dataset` function iterates through the dataset with a
  ...
  PIL’s `save()` (writing to an in-memory `BytesIO` buffer as JPEG). We
...
- Every `batch_size` images (here 1024), we convert the lists to a PyArrow Table
  ...
  This two-level batching ensures we don’t hold all data in memory: at most
...
Now we can load the dataset to do model training. Next, we’ll see how to load
...
- `LanceDataset` is an **iterable-style** or **streaming** dataset (akin to
  ...
  than one GPU in the mix) without needing PyTorch’s `DistributedSampler`.
- `SafeLanceDataset` is a **map-style** or **in-memory** dataset (akin to
  ...
  production workflows start out with this approach, and it’s best for
  fixed-size datasets because it can yield better performance with standard
  `DataLoader` shuffling and sampling.

In short, use a map-style `SafeLanceDataset` for data that will never change. If
the dataset is continuously growing (such as a continuously updated dataset of
social media posts) or is so large that even the index of it can’t fit on a
single machine (such as a dataset of the entire set of English language YouTube
videos), use an iterable `LanceDataset`.

We’re going to show off what it looks like to use both options. In either case,
the goal is to decode image bytes into tensors and apply any preprocessing
transformations we need for training. We’ll use a simple transformation: resize
...
the raw bytes can’t be directly turned into Torch tensors without conversion.
...
`{“image” b”...”, “label”: ...}` and performs the decoding and stacking:
...
also use PyTorch’s `DistributedSampler` if training in distributed mode (see
next section), or Lance’s own fragment samplers (for Iterable datasets). In the
...
- With `LanceDataset` (iterable), Lance provides its own `ShardedBatchSampler`
  ...
  every process read the entire dataset; this is exactly what you don’t want for
  training. It’s important to use shard-aware samplers in distributed mode with
  `LanceDataset`.
...
In contrast, here’s what you’d do with `SafeLanceDataset`:
...
Tigris’ bottomless object storage and global availability. The LanceDB Python
