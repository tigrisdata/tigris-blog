---
slug: northflank-docker-registry
title: "1-Click Docker Registry with Northflank and Tigris"
description: |
  Deploy your own private Docker registry in minutes with Northflank and Tigris. Get unlimited container storage, no egress fees, and global distribution for your Docker images without the complexity of managing registry infrastructure.
...
<img
  ...
  alt="A cartoon tiger taking a picture of the Golden Gate bridge."
/>

<center>
  <small>
    <em>A cartoon tiger taking a picture of the Golden Gate bridge.</em>
  </small>
</center>
...
To get started, click here \<link\>. The rest of this article is going to
explain the moving parts in this template and how they all work together.
...
```text
openssl req -x509 -nodes -new -sha256 -days 36500 -newkey rsa:4096 -keyout registry.key -out registry.pem -subj "/C=US/CN=Registry Auth CA"
```
...
```text
$ docker run --rm -it http--registry--5yk6bhbvjh58.code.run/base/alpine:edge cat /etc/os-release
NAME="Alpine Linux"
ID=alpine
...
<InlineCta
  title="Ready to deploy your own Docker registry?"
  subtitle="Get unlimited container storage with no image egress fees using Northflank and Tigris."
  button="Deploy now"
  link="https://github.com/tigrisdata-community/self-hosted-docker-registry"
/>
