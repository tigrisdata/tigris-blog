---
slug: mcp-oidc-provider
title: "We're open sourcing our MCP OIDC Provider"
description: |
...
import heroImage from "./mcp-oid.webp";

<img
  src={heroImage}
  className="hero-image"
  alt="We're open sourcing our MCP OAuth Provider"
/>
...
## MCP Protocol Requires Discovery (OAuth or OIDC)
...
- [https://mcp.storage.dev/.well-known/oauth-authorization-server](https://mcp.storage.dev/.well-known/oauth-authorization-server)
- [https://mcp.storage.dev/.well-known/oauth-protected-resource/](https://mcp.storage.dev/.well-known/oauth-protected-resource/)

## Dynamic Client Registration: Upstream IdPs alone don't solve this
...
flowchart TD
    subgraph CLIENT_SIDE["Client Side"]
        MCP_CLIENT["MCP Client<br/>(Claude, Cursor, ChatGPT, etc.)"]
    end

    subgraph AUTH_LAYER["Auth Layer"]
        OIDC_PROVIDER["mcp-oidc-provider<br/>(OIDC Authorization Server)"]
        UPSTREAM_IDP["Upstream IdP<br/>(Auth0, Clerk, Okta, etc.)"]
    end

    subgraph RESOURCE_LAYER["Resource Layer"]
        MCP_SERVER["MCP Server<br/>(OAuth 2.1 Resource Server)"]
    end

    MCP_CLIENT -->|"OIDC Discovery<br/>Client Registration<br/>/authorize /token /jwks"| OIDC_PROVIDER
    OIDC_PROVIDER -->|"OIDC Federation<br/>(User authentication, claims)"| UPSTREAM_IDP
    MCP_CLIENT -->|"MCP Requests with<br/>Bearer Access Token"| MCP_SERVER
    MCP_SERVER -.->|"JWT Verification via<br/>/jwks & token introspection"| OIDC_PROVIDER
```

</center>

### Usage: Referencing the Standalone Implementation
...
### Usage: Integrated Mode (Express + MCP)
...
- Exposes a single BASE_URL you point your MCP client at
...
BASE_URL must be the public URL clients will use for:
...
async function main() {
  ...
  // Configure upstream IdP client (Auth0 / Clerk / Okta / Keycloak / etc.)
  const idpClient = new OidcClient({
    ...
    // optional: scopes, additionalAuthParams, extractCustomData, ...
  });

  // Persistent store for sessions, grants, tokens, etc.
  const store = new Keyv(); // or new Keyv({ store: new KeyvTigris() }) in prod

  // This sets up:
  // - OIDC endpoints (/authorize, /token, /jwks, /.well-known/openid-configuration, /register)
  // - MCP auth wiring
  // - A helper to register your MCP handler
  ...
  // Your MCP handler: runs after a valid, authenticated request.
  // req.user contains the authenticated user (from upstream IdP claims).
  handleMcpRequest(async (req, res) => {
    console.log("Authenticated user:", req.user);

    // Implement your MCP server here:
    // - parse req.body for MCP messages
    // - respond with MCP-compliant responses

    // For now, return a simple MCP-style stub:
    res.json({
      jsonrpc: "2.0",
      id: req.body?.id ?? null,
      result: {
...
<InlineCta
  title="Try mcp-oidc-provider"
  subtitle="Ready to build a secure MCP server? Get started with mcp-oidc-provider and use any OIDC-compatible IdP you want."
  button="Check out the repo"
  link="https://github.com/tigrisdata/mcp-oidc-provider"
/>
