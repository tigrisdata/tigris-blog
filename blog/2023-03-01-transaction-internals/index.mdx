...
Tigris is a developer data platform that offers an open source and
scalable alternative to MongoDB Atlas. Tigris utilizes
...
Tigris executes operations across collections to perform multiple dependent operations within a transaction. The user
initiates a transaction by submitting a **BeginTxn** request and controls the transaction state using **Commit** or **Rollback**.
...
The following example illustrates how easy it is to leverage Tigris’s transactions across collections and documents without any
restrictions.
...
and isolation. For instance, [Spanner uses True-time](https://www.usenix.org/system/files/conference/osdi12/osdi12-final-16.pdf)
for determining the commit timestamps when all locks are acquired. [CockroachDB
uses the hybrid-logical clock](https://dl.acm.org/doi/pdf/10.1145/3318464.3386134), a combination of physical and logical time. Tigris provides lock-free concurrency
...
> In most cases, multi-document transaction incurs a greater performance cost over single document writes, and the availability
> of multi-document transactions should not be a replacement for effective schema design.
...
operations with a query filter that uniquely identifies a single document. Quoting below from [MongoDB's manual](https://www.mongodb.com/docs/manual/reference/read-concern-linearizable/#performance-comparisons):
...
the performance implications, you will frequently end up working around the use of transactions in MongoDB.
On the contrary, Tigris provides serializable transactions by default, without any additional overhead for
cross-shard, cross-collection transactions, and doesn’t require tunable options so that performance
