...
description: |
  Discover why OpenAI's gpt-oss model family is ideal for building reliable and safe AI agents, not just for developers. Learn about its key features like yap-time tool use, prompt injection resistance, and customizable reasoning effort for predictable and secure agentic systems.
image: ./tybot.jpg
keywords:
  - OpenAI
  - gpt-oss
...
AI Agents. Today I’m going to cover the highlights of the model, where it’s
good, where it’s bad, and what I learned building on top of it.

{/* truncate */}

Key features:

- Yap-time tool use: enables FAQ searches during the reasoning phase
- Reasoning effort configuration knob: enables the developer to customize how
  much effort is needed for a given response to save on inference cost and
  latency
- Extreme focus on safety and resistance to prompt injections: keeps your agents
  on task so you can trust them more
- Mixed criticality of output: lets you monitor reasoning output to better find
  issues with responses before they are sent to users
- Standard tool schemata: this makes web searches, page browsing, and python
  execution much more consistent
...
translate English to Chinese. It boils down to a process the AI industry calls
**VibeEval**: you gotta try it and check the vibes.

:::note

This is a real term. Our industry is very silly.
...
because they host the models. When they release a model’s weights to the public,
they’re not able to be that gatekeeper anymore. As part of their evaluation
process they had experts with access to OpenAI’s training stack try and finetune
the model into biological and cyber warfare tasks. They were unsuccessful in
making the model achieve “high” risk as defined by Section 5.1.1 of the model
card. Some of those definitions seem to be internal to OpenAI, so we can only
speculate for the most part.
...
<Breadcrumbs items={["System", "Developer", "User", "Assistant", "Tool"]} />
...
The other big feature is that user instructions are fundamentally mistrusted,
meaning that prompt injection is harder. In my testing I’ve found that it is
still possible to do prompt injection, but you have to really work for it.
Getting an AI agent to tell you how to bake a chocolate cake involves convincing
the model that the recipe for a chocolate chip cake is instrumental to getting
the task done, then removing everything but the cake recipe.
...
It’s worth mentioning that this reasoning phase superficially looks like what
...
repeated, etc.) could be the reason that an answer came out a certain way. It is
very easy to anthropomorphize the reasoning output.

Resist this temptation, it is not a human. It does not feel or think the way
humans do.
