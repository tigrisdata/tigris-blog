...
<div className="mermaid-frame">
  TODO(Xe): make diagram showing the "racing the beam" effect of fetching
...
As I mentioned earlier, there’s a bunch of hype around making different ways of
storing git repositories outside of the context of putting everything on
filesystem files like the good old days. A lot of them seem to commit a
distributed systems sin that’s kind of hard to work around: I think they’re
being vastly over-engineered for the kind of problem Git actually is.

One of the things that sucks about Git is that its data format is yada yada etc
compare with cursor and walgit etc

NOTE: walgit has a separate process for compaction and noted that they can’t
easily do range requests over packfiles, maybe they hit the same problem I did?
