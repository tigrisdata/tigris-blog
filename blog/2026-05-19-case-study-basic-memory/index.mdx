...
more granular feature: file-level diff/merge.

Because every change to the bucket is persisted with a snapshot ID — similar to
a version SHA — Basic Memory can fetch every version of a Markdown file and load
the changes side-by-side in a CodeMirror MergeView. A `ListObjectVersions` call
...
As Paul describes, "I had experimented with integrating Git for this and it was
...
the file history for a single file."
...
Tigris is the good parts of AWS without the bad parts.

</BreakoutQuote>

He wanted to build differently. Tigris fit. Per-tenant buckets, zero egress,
