...
title: Tigris Flow Part One - Modelling one to one relations
description: >
  ...
  demonstrates how to model one to one relations using either embedded documents or referencing another collection.
keywords: [relations, relationships, data modelling, typescript]
authors: [garren]
tags: [mongodb alternative, relations, relationships]
---

Tigris provides a transactional semi-structured document database, this allows you to model relations in a few different ways to get the best performance, consistency and ease of use in your application.

In this blog, I show you how to get up and running with Tigris and how to model one-to-one, one-to-many and many-to-many documents using the embedded document and sub-document pattern. We will combine this with Tigris's transactions to make sure that all updates are consistent.
...
This creates a `Location` object which is embedded inside the `User` schema. This allows us to store and read the `Location` and `User` in one query. The User's email address will be the primary key.
...
Then our `main` function we get the database object and then fetch our `user` collection object so that we can work with our collection. We create 3 new users. And then show how to query for one on line XX or if we want to query many of them based on a filter we do that on line XXX.

## Reference one-to-one relationships

There can be cases where our embedded objects are very large and we do not need to load them every time. In these cases, we can split the document into its own collection and fetch it when we need it. Now we can add a short `biography` about the users we have added above and put it in a separate collection so that we can load it when we need to.

We are going to add a new `biography` object in our `user.ts` file so that our `user.ts` file now looks like this:
...
We have added the `biography` as a collection in the `setup` function. Then in the main, we created a transaction before we insert out `user` and `biography`. We pass this transaction to both `insert` functions. We use a transaction here so that both insert operations will either complete successfully or the transaction will roll the inserts back. This makes sure that writing to both collections is safe and consistent. We never have to worry about our data not being accurate or correct.

Now we can read from the two collections. We do this at Line XXX. We start another read transaction and first read from the `userCollection` as we stream those results back we also fetch the `biography` of the user from the `bioCollection`. Again we use a transaction to make sure that we are reading a consistent snapshot of the data and that no modifications will affect the queries.
