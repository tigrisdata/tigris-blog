...
Let’s say I wanted to run data analytics on data stored in Supabase, but some of
the data I need to compare against is in Google Cloud Storage (GCS). I’d have to
pay by the gigabyte if I had the gall to run that query from anywhere but Google
Cloud. I can’t magically teleport my laptop to be inside Google Cloud’s network
(yeah, they have a VPN product, but you still have to pay egress for that) and
my neocloud workloads definitely aren’t in Google Cloud, so I’d have to restrict
all of my planning to find ways to lock myself deeper into the clutches of that
ecosystem.
