...
The [`tigris` CLI](https://www.tigrisdata.com/docs/cli/bundle/) wraps the same
...
The keys can come from a file (one per line) or stdin instead of the flag, and
you can compress the stream on the way out with `--compression gzip` (or
`zstd`):
...
Pass `--on-error fail` to make a missing key abort the whole bundle instead of
getting skipped and logged in `__bundle_errors.json`.
...
<video controls autoPlay loop muted playsInline width="100%">
  <source src="/blog/img/blog/bundle-api/bundle-demo.mp4" type="video/mp4" />
  Download the <a href="/blog/img/blog/bundle-api/bundle-demo.mp4">MP4</a> version.
</video>
