...
Veo3 dramatically lowers the barrier to creating high‑quality video. Here’s a sample I generated with this prompt:

<prompt>
Cinematic. An Over-the-Shoulder Shot slowly zooming in on a dark interview room. A detective is intensely interrogating a rubber duck. The prominent sound of a ticking clock fills the tense air as the detective sternly asks, "Where were you last night?"
</prompt>
...
It’s not perfect (yet), but it’s impressive—a big leap for video generation. Even at ~$3 per generation today, it’s far cheaper than a full video setup. Expect a lot more video content to be model‑generated in the near future.

In this tutorial, you’ll build a text‑to‑video app powered by Google’s Veo3. You can follow along in Python (Jupyter Notebook) or TypeScript (Next.js).
...
### Jupyter Notebook
If you prefer Python, follow the step‑by‑step Jupyter Notebook in the accompanying GitHub repository.

### Next.js
If you prefer TypeScript, clone the example repository and follow the instructions. You can use the `03-enhance-prompt` branch if you don’t want to implement the Veo and Tigris code yourself. Project structure:
...
1) Create a bucket at `https://storage.new` (you’ll be prompted to create a Tigris account if needed). Name it, e.g., “veo3-generations”, and click Create.
2) Create an access key with Read/Write. Toggle “Environment variables” and copy them into your Next.js `.env`.
