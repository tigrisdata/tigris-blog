...
chosen geography (for example `usa` or `eur`). You select the geography (USA or
EUR), Tigris automatically selects the regions inside that geography, and
maintains two or more copies. Every read and write inside that geography behaves
...
without hand-wiring replication between single-region buckets.

Multi-region buckets are priced at **$0.025/GB/month**, regardless of how many
regions Tigris uses inside the geography, and egress is still free. See the
...
needs to match, or when you want a small, explicitly pinned set of regions. It
also works as a warm DR setup: a second copy in a known location without running
your own replication service. Pricing stays simple here too: you pay a single
per-GB storage rate with zero egress fees, not separate line items for each
region or replica.

For example, an EU dual-region bucket might be:

<DiagramFrame title="Dual-region bucket (fra,lhr)">

```mermaid
flowchart TD;
  D["Dual-region bucket (fra,lhr)"]
  fraNode["fra (Frankfurt)"]
  lhrNode["lhr (London)"]
...
| Dual-region   | Per pricing page (standard tier) | Free        | Same per-GB rates across geos  | Included (no per-region add-ons) |
| Single-region | Per pricing page (standard tier) | Free        | Same per-GB rates across geos  | Included (no per-region add-ons) |
| Free tier     | 5 GB standard storage + requests | Free        | Global                         | Included                         |

:::tip Pricing at a glance

For the full breakdown, including storage tiers and request pricing, see the
...
<InlineCta
  ...
  link="https://console.tigris.dev/signin"
/>
