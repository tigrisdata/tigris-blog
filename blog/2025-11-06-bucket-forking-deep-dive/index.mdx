...
title: Copying of terabytes of data in microseconds with time travel
description: |
...
future. One of the easiest ways to think about snapshots is via a linked list
recording the changes to an integer counter:

TODO: Live diagram: a counter app vsplit by a view of the history of changes as
a list. The snapshot button makes a sparkle emoji button show up next to the
current change, when you click that button after making changes, the counter app
and list revert back to the state as of that snapshot.

import TODODiagram from "./TODODiagram";

<TODODiagram />

Instead of just recording a single counter value, the snapshotting approach has
you record the changes to that counter and set flags on events so you can get
back to where the counter was at that point in time. There’s a few ways to
implement this, but for now let’s assume this is implemented as an
append-only-log: or a data structure where the only event you’re allowed to take
is appending new data on the end of it.

:::note

From here you can implement things like a “CurrentState” event that represents
the integer state of the counter by reducing/compacting all of the changes to
the counter to a single integer value and then use that in the future. This is
trivial and thus an exercise for the reader.
...
[Postgres implements its replication model](https://www.postgresql.org/docs/current/wal-intro.html),
how
...
Postgres’ Write-Ahead-Log files contain the list of changes made to an empty
database and use them to recreate the state of database tables. ZFS internally
represents its changelog as a list of changes made to an empty block storage
device and use them to recreate what the contents of the disks should be. Git is
a list of patch files applied to an empty directory and uses those patches to
recreate the state of your source code tree. If you need to read data at an
earlier point in time (such as the state of a source code tree at a given git
commit hash or tag), you can ask your git client to uncompute the changes needed
to take you there.

:::note

The log is the database. Anything else is just a cached view of that log.
...
data forever. Comparatively, Tigris lets you put objects into a state of
[quantum immortality](https://en.wikipedia.org/wiki/Quantum_suicide_and_immortality)
where even if you “delete” the data you can get it back as long as you have a
...
TODO: time.Now().UTC().UnixNano() in JS, update on refresh or when user clicks
refresh button
...
TODO: compute three timestamps 3 days apart each but out of order, side by side
/ table view of what they look like

The really cool part happens when you sort them:

![A diagram showing how the most recent version is sorted to the top](./img/revsort-time.webp)

This is inherently ordered such that the most recent timestamp will always be
sorted to the top. So when the Tigris gateway goes in and asks for an object, it
asks FoundationDB to return the value of the key that comes after
`bucket-name/object-name`, and then whenever objects are created, updated, or
deleted, Tigris creates a new version of the object metadata and inserts it on
top of the existing object metadata. All the older versions of objects are
neatly frozen in time.
...
TODO: diagram where the timeline forks on each fallible event

This basic idea is how we handle bucket forking. When you create a new bucket
forked out of an existing snapshot, you basically create a new timeline for that
bucket that diverges from the bucket’s old timeline:
...
TODO: diagram of that above flowchart
...
TODO: tigris SDK example of putting a value into a bucket, then taking a
snapshot and creating a fork of that snapshot.
...
TODO: tigris SDK example of deleting the data in the child bucket

Now the child bucket is empty. But, we can still view the data in the parent
bucket:

TODO: tigris SDK example of fetching the data from the parent bucket even though
it can’t from the child bucket
...
TODO: diagram of child being able to access data before a deletion tombstone in
the parent
