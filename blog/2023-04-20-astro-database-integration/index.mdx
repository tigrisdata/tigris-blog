---
slug: astro-database-integration
title: Adding NoSQL to Your Astro Site
description: >
  Introducing the Tigris Data integration for Astro, and how to add your NoSQL
  data to your static or server rendered websites.
keywords: [jamstack, nosql]
authors: [michael]
tags: [jamstack, nosql]
...
sites. Having built several sites with Astro, I can attest to the breadth
of options to get your site up and running quickly.

But there was one noticeable gap in the ecosystem: database access. This
is understandable because most Astro sites are built to be static. That's not to
...
That's why we're excited to announce the official Tigris Data integration
([@tigrisdata/astro](https://npmjs.org/package/@tigrisdata/astro)) for Astro.
This integration allows you to use Tigris Data as your database, and pull your
data into your Astro site. This means you can build a static site with Astro,
and use Tigris Data to power your content.

<!-- truncate-->

## How to Use the Tigris Data Integration for Astro

The Tigris Data integration for Astro is available on npm. To install it, run:

```bash
npm install @tigrisdata/astro
```

Once installed, you'll need to configure your Tigris Data connection in your
...
:::tip
If you add the following environment variables, the config options aren't required
...
That code not only pulled data from a "comments" collection but also did it in
a type-safe way. That's right! Even though Tigris is a NoSQL database, your
data can still be typed and validated.
:::

## What Could I Build with Tigris and Astro?

The short answer is "don't let your dreams be dreams." 😁 The long answer is
the possibilities are nearly endless. Here are a few ideas (and a sample app)
to get you started:
...
Basically, I use the base Astro blog template and added:
...
If you have additional ideas or want to tackle one of these, jump on over to
the [GitHub repo](https://github.com/tigrisdata-community/astro-comments) and
submit a PR. We'd love to see what you come up with.
