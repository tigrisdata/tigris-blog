...
<Tabs>
<TabItem value="debuntu" label="Debian/Ubuntu" default>

Download the `.deb` package from
[the most recent release](https://github.com/tigrisdata/tigrisfs/releases) and
then put it in a local folder. Install it with `apt install`:

```text
sudo apt install ./tigrisfs-version.deb
```

</TabItem>
<TabItem value="rpm" label="Alma Linux / Fedora / Red Hat / Rocky Linux">

Download the `.rpm` package from
[the most recent release](https://github.com/tigrisdata/tigrisfs/releases) and
then put it in a local folder. Install it with `dnf install`:
...
Then open `/etc/default/tigrisfs` in your favorite text editor as root and do
the following:

- Uncomment the `AWS_ACCESS_KEY` and `AWS_SECRET_ACCESS_KEY` variables and paste
  in the access key you got from the admin console.
- Uncomment the `AWS_ENDPOINT_URL` variable so tigrisfs points to Tigris
...
Your bucket will be available at `/mnt/tigris/bucketname`. If you need things to
be writable by your user account, edit the `OPTS` line based on your account’s
...
to leverage Tigris-specific features that improve throughput and latency. In
essence, tigrisfs bridges the gap between the Linux kernel and Tigris. It
...
For most file operations, tigrisfs will be more than enough. If you need special
Unix features like Unix sockets, character devices, or other magic things like
extended attributes, tigrisfs won’t work out for you.
...
Go out there and make stuff! We’ll have your back.
