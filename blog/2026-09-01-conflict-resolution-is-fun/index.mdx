...
flowchart TB
    ...
    subgraph row2 [" "]
        direction LR
        BLK["Upload the bytes to the owning<br/>region's block store"] --> META["Commit the metadata over gRPC to the<br/>owning region's FoundationDB cluster"]
        META --> FDB[("FoundationDB<br/>owning region")]
        FDB --> OK["Return success"]
    end
    subgraph row3 [" "]
        direction LR
        W["Replication worker"] --> OTHER["Every other region gets<br/>a read-only copy"]
    end

    row1 --> row2
    row2 -.->|reads queue| row3
