...
<MermaidFrame style={{ maxWidth: "26rem", margin: "1.5rem auto" }}>

```mermaid
flowchart TD
    C["Client<br/>PutObject"] --> GW["Nearest Tigris gateway"]
    GW --> Q{"Which region owns<br/>this bucket?"}
    Q --> BLK["Upload the bytes to the owning<br/>region's block store"]
    BLK --> META["Commit the metadata over gRPC to the<br/>owning region's FoundationDB cluster"]
    META --> FDB[("FoundationDB<br/>owning region")]
    FDB --> OK["Return success"]

    FDB -.->|reads queue| W["Replication worker"]
    W --> OTHER["Every other region gets<br/>a read-only copy"]
```
...
```mermaid
flowchart TD
    Proto["compare(previous,<br/>new, force)"]
    IsNewDeleted{"Is prev or<br/>next unset<br/>or a new<br/>key in FDB?"}
    IsPrevOlderThanNew{"Is prev older<br/>than the new<br/>data?"}
    IsTiebreaker{"Is this a<br/>forced change?"}
    T["return true<br/>(Apply, row replaced)"]
    F["return false<br/>(Drop, log the conflict)"]

    Proto --> IsNewDeleted
    IsNewDeleted -->| Yes: different objects | F
    IsNewDeleted -->| No: same key | IsPrevOlderThanNew
    IsPrevOlderThanNew --> | No: new is older | IsTiebreaker
    IsPrevOlderThanNew --> | Yes: new version is newer | T
    IsTiebreaker --> | Yes: tie forcibly broken | T
    IsTiebreaker --> | No: new version is older<br/>or tie without force | F

    style T fill:#d4edda,stroke:#28a745,color:#0f5132
    style F fill:#f8d7da,stroke:#dc3545,color:#842029
```
...
```mermaid
flowchart TD
    REQ["S3 request"] --> GW["Gateway"]
    GW -->|"which replicaset<br/>holds this tenant?"| SM[("Shard mapping<br/>cluster")]
    SM -.->|"replicaset 2"| GW

    GW ==> RS2

    subgraph RS1["Replicaset 1"]
        C1[("2-3 FoundationDB clusters")]
        W1["Worker pool"]
        C1 --- W1
    end

    subgraph RS2["Replicaset 2"]
        C2[("2-3 FoundationDB clusters")]
        W2["Worker pool"]
        C2 --- W2
    end

    subgraph RS3["Replicaset 3"]
        C3[("2-3 FoundationDB clusters")]
        W3["Worker pool"]
        C3 --- W3
    end

    C1 --> GT["Global Pointer<br/>table"]
    C2 --> GT
    C3 --> GT

    style GT fill:#f8d7da,stroke:#dc3545,color:#842029
```
