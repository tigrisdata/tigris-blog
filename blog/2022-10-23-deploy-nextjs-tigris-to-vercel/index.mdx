...
description: >
  ...
  deploy it on Vercel.
keywords: [open source, web application, nextjs, vercel, deployment, typescript]
authors: [adil, tkhan]
tags: [nextjs, vercel, fullstack, web development, typescript, deployment]
---
...
### 3.2 `lib/tigris.ts` - Connecting to Tigris {#code-connecting-to-tigris}
...
Tigris operations from here on. Also, note how we are caching the client instance that it can
be subsequently used for multiple requests without having to reconnect to Tigris.
...
Let's start with the `/api/items` api that supports both `GET` and `POST` handler. You'll realize
that Next.js eliminates a lot of REST boilerplate and allows developers to focus more on the
functional aspects of the application.

<!-- prettier-ignore-start -->
```typescript title=/pages/api/items/index.ts
...
`handleGet()` method is fetching and returning items from Tigris collection, let's take a look at its
implementation
...
In this code, we imported `tigrisDb` from [/lib/tigris](#code-connecting-to-tigris) and re-used it
to establish a communication with Tigris. Tigris has collections, and each collection has
multiple documents. Tigris typeScript client provides a very simple abstraction from the data layer
that is easy to comprehend for the developers operating on large code-bases. `itemsCollection.findMany()`
follows native coding convention to send a query to Tigris DB equivalent to **SELECT \*** statement of
SQL and returns a cursor to fetch results from collection.

Let's look at `handlePost()` implementation in the same file that inserts a `TodoItem` in collection
...
`handlePost()` gets called on `POST /api/items` and expects a TodoItem object as json in body. It
leverages another simple `insertOne()` construct on the collection object to execute SQL equivalent
**INSERT INTO** query.

## Summary

In this tutorial, you deployed a To-do list web application using Tigris and [Next.js][nextjs-url]
framework on [Vercel][vercel-url], then we saw how easy it is to setup Tigris client in your Next.js
app. We saw how Next.js and [Tigris][tigris-url] together provide simple interfaces for developers
to adopt and get going with their business logic. You can find the complete source code for this tutorial
[GitHub repo][tigris-todo-repo-url], feel free to raise issues or contribute to the project.
Happy learning!
