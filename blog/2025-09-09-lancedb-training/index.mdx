...
[`LanceDataset`](https://docs.lancedb.com) treats the dataset as a stream of
data. It reads batches of data directly from the LanceDB store instead of
relying on the PyTorch `DataLoader` class. In our case, `LanceDataset` will
stream from the Lance file we created on Tigris and yield one batch at a time.
