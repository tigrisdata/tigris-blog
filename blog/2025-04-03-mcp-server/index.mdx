...
<Tabs>
<TabItem value="config" label="Step 1: Build the Image" default>

In order to build the image, first clone the repository:

```text
https://github.com/tigrisdata/tigris-mcp-server
```
...
<TabItem value="script" label="Step 2: Configure Client">

Either you can run the init script in your terminal and select Docker as option
when prompted:
...
or manually configure the `claude_desktop_config.json` or `mcp.json` for Cursor
AI
