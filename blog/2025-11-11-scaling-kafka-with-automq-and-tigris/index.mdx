...
import heroImage from "./kafka-hero.webp";

<img
  src={heroImage}
  className="hero-image"
  alt="Scaling Kafka with AutoMQ on Tigris"
/>

# Scaling Kafka with AutoMQ on Tigris

Apache [Kafka®](https://kafka.apache.org/) has been the standard for event
streaming and building reliable, high-throughput, event-driven systems. However,
as Kafka deployments scale across clouds and regions, its core architectural
choices start showing limits.
...
[AutoMQ](https://www.automq.com/) is a fully Kafka-compatible streaming engine
that re-implements the Kafka broker while preserving full protocol
compatibility. Its defining architectural change is delegating persistence
entirely to object storage like Tigris via the S3 API.
