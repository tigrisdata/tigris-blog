...
and will never change. If the dataset is continuously growing (such as a
continuously updated dataset of social media posts) or is so large that even the
index of it can’t fit on a single machine (such as a dataset of the entire set
of English language YouTube videos), use `LanceDataset`.
