...
TAG at ~200x — confirming that a correctly configured S3-native pipeline is
...
can compensate — the goal being to keep enough requests in flight that the GPU
stays fed despite per-request latency.
...
sequential (sharded). With that context, here is how the benchmark was
configured.
...
cost of per-object TTFB at scale — 16 workers needed just to feed one GPU. Now
the natural question is whether packing images into tar shards meaningfully
...
Note that, both Part 1 and Part 2 are single-epoch benchmarks, they measure the
steady-state performance of training directly off object storage. But most real
training jobs are multi-epoch: the same dataset is iterated over many times.
That changes the calculus significantly, and that's where caching becomes
relevant.
...
cache. From the perspective of the training process, it is an ordinary S3
endpoint — no changes to training code are required beyond pointing the endpoint
URL at `localhost:8080` instead of `t3.storage.dev`.
...
The ViT benchmark answers the practical question: can the data pipeline keep the
GPU fed? Parts 1-3 all answer yes. But they don't reveal how much margin exists
in the data loading layer, the GPU saturates before the storage layer is tested
to its limit. To find that ceiling, we need to remove the GPU from the equation
entirely.
...
every shard size tested — the storage layer had substantial remaining headroom.
...
per-request overhead of S3 GET calls is negligible; the binding constraint
shifts to how well work is parallelized across the 8 dataloader workers. Smaller
shards distribute more evenly. At 64 MB shards, CPU p90 reaches 91.3% — TAG
shares the instance CPU with the PyTorch workers, and at larger shard sizes the
combined load begins to constrain throughput.

With all four benchmarks complete, here is what the results look like side by
side.
...
count. Crucially, TAG achieves this on a raw unsharded image dataset, training
directly on individual S3 objects, with no tar packaging or dataset
preprocessing. For workloads that iterate over the same data across multiple
epochs, this removes a significant operational burden.
