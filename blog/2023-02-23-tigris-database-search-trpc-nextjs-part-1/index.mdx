---
authors: [leggetter]
tags: [tigris database, tigris search, nextjs, trpc]
---

# Using Tigris Database and Search with tRPC and Next.js - Part 1

As we continue to build out our [open source Developer Data Platform](https://github.com/tigrisdata/tigris) with data services such as [Database](https://www.tigrisdata.com/docs/concepts/database/) and [Search](https://www.tigrisdata.com/docs/concepts/searching/), we're constantly exploring ecosystems and their technologies to determine where our platform solves the biggest problems and is a natural fit. One of the very obvious and very big ecosystems is full-stack web application technologies built with JavaScript. When reviewing [the JavaScript Rising Stars most popular projects](https://risingstars.js.org/2022/en#section-all) there are a couple of standout technologies that enable developers to build full-stack web apps:

- [Next.js](<[https://nextjs.org/](https://nextjs.org/)>)
- [tRPC](https://trpc.io)

So, it made a lot of sense to explore using Tigris Database and Search with tRPC and Next.js.

In this three-part tutorial series, we'll demonstrate how to build a simple social network type application called “Stream Social”.

**// TODO: webm of the completed application to create some level of excitement**

The application will be pre-loaded with 10,000 posts across a number of different users so that we're working with a reasonable dataset. We'll build functionality that lists the posts and allows you to paginate through them. We'll also build functionality that lets you view all posts from a specific user and perform full-text search on all the posts.
...
- [Node.js LTS](<[https://nodejs.org/en/download/](https://nodejs.org/en/download/)>) or above which comes with the [NPM CLI](<[https://docs.npmjs.com/cli/v9/commands/npm](https://docs.npmjs.com/cli/v9/commands/npm)>)
- [Git](<[https://git-scm.com/book/en/v2/Getting-Started-The-Command-Line](https://git-scm.com/book/en/v2/Getting-Started-The-Command-Line)>) so you can clone the code required for the tutorial or check out the completed code.
...
### Create a Tigris Project and configure the app
...
Next, define a `Post` model in a new file, `db/models/user.ts`:
...
And you will see output as follows:
...
3. We call `useEffect` with `[queryPosts.data](http://queryPosts.data)` as a dependency. This means the effect is only run if the value of `queryPosts.data` changes and this is where the post data is stored. If the `status` of the query is `success` then we call `setPosts(queryPosts.data as Post[]);` to set the value of the `posts`.
...
4. The `createdAt={post.createdAt!}` has an eslint comment because it could be `null` based on the class definition. However, since we've pulled the posts from the Tigris Database where the value is auto-populated, we know it won't be null.

Take a look at the source for `<PostCard />` in `src/components/post-card.tsx` if you like. It uses several MUI components. There's also a few things we don't fully use, such as the like/heart and share icons, but we've left those in to make the app feel more social and as an opportunity for you to add more functionality.

The Next.js application is now contains a user and posts from the Tigris Database, retrieved using tRPC queries.
...
1. How to setup a Tigris Project, define a Tigris Database schema using data models defined in TypeScript, and setup a Next.js project to make use of Tigris
