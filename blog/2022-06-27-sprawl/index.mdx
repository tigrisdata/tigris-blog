...
The backend though has become a distributed system with many different
...
the development of a modern database platform that would keep the data
...
1. A flexible document model that enables developers to model data in a way
   that best suits the applications’ needs. The data model should also be easy
