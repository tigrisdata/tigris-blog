...
create it, as `FORK`, `TIGRIS_ACCESS_KEY_ID`, and `TIGRIS_SECRET_ACCESS_KEY`.
Those are the only credentials the sandbox ever holds, which means the base
bucket isn't hidden from your agent; it's unreachable: a prompt injection, a
buggy tool, or a model on a bad day can name `agent-world` all it wants and the
API will refuse it. Inside the snapshot, the entrypoint does the rest:
...
Be honest about the asymmetry here: the fork was free, and promotion isn't. It's
a copy, the one step in the loop that costs what it weighs, so promote the
...
3. In the harness, before each sandbox boots: fork the base
   ...
   and pass them into the sandbox env as FORK,
