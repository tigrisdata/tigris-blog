---
slug: s3-compat
title: "We're S3 compatible so you don't have to change your code"
description: >
...
systems. The S3 API is a standard because of its simplicity. Here’s what the
...
over to enqueue, transcode, and publish the optimized files so people can sit
...
to set up an FTP server with your own subversion or CVS repo. Users need to
store more and more files as tech exploded in growth during the 2010s. S3 let
them ship Dropbox to users faster; imagine how much time it would have taken if
they had to provision their own hardware and write their own storage backend
like SmugMug did.
...
vast majority of workloads. Simply add access keys, set the region, and change
your endpoint URL to point to Tigris, and your code will Just Work™. We have
[comprehensive coverage of the S3 API](https://www.tigrisdata.com/docs/api/s3/),
and we work with all your familiar tools. Our goal is to enhance the object
storage experience by adding features we always wished we had like
[on-demand data migration](https://www.tigrisdata.com/docs/buckets/settings/#data-migration)
and dynamic placement of objects all over the globe. All whilst remaining S3
compatible so you can easily mix and match cloud providers and services.
