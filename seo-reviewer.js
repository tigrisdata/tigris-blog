...
class SEOReviewer {
  ...
  constructor() {
    ...
    this.htmlImagePattern = /<img[^>]*src=["']([^"']+)["'][^>]*(?:alt=["']([^"']+)["'][^>]*)?>/g;
  ...
  validateFilePath(filePath) {
    ...
    if (!resolvedPath.startsWith(resolvedBase) && !resolvedPath.startsWith(process.cwd())) {
  ...
  findNearbyCaption(lines, imageLineIndex) {
    ...
    for (let i = imageLineIndex + 1; i <= Math.min(imageLineIndex + 3, lines.length - 1); i++) {
      ...
      if (line && !line.match(/^<\/?em>/) && !line.match(/^[*_]/) && !line.startsWith('![')) {
        break;
      }
    }
  ...
  performSEOAnalysis(analysis) {
    ...
    if (!frontmatter.title) {
      ...
      if (titleLength < SEOReviewer.TITLE_MIN_LENGTH) {
        analysis.suggestions.push(`Title is short (${titleLength} chars). Consider expanding to 50-60 characters for better SEO`);
      } else if (titleLength > SEOReviewer.TITLE_MAX_LENGTH) {
        analysis.suggestions.push(`Title is long (${titleLength} chars). Consider shortening to under 60 characters`);
      }
    }
    ...
    if (!frontmatter.description) {
      ...
      if (suggestedDesc) {
        analysis.suggestions.push(`Suggested meta description: "${suggestedDesc}"`);
      }
    } else {
      const descLength = frontmatter.description.length;
      if (descLength < SEOReviewer.DESCRIPTION_MIN_LENGTH) {
        analysis.suggestions.push(`Meta description is short (${descLength} chars). Expand to 150-160 characters`);
        // Generate suggested improved description
        const suggestedDesc = this.generateImprovedDescription(analysis);
        if (suggestedDesc) {
          analysis.suggestions.push(`Suggested improved description: "${suggestedDesc}"`);
        }
      } else if (descLength > SEOReviewer.DESCRIPTION_MAX_LENGTH) {
        analysis.suggestions.push(`Meta description is long (${descLength} chars). Shorten to under 160 characters`);
        // Generate suggested shortened description
        const suggestedDesc = this.generateImprovedDescription(analysis, true);
        if (suggestedDesc) {
          analysis.suggestions.push(`Suggested shortened description: "${suggestedDesc}"`);
        }
      }
    }
    
    // Keywords analysis
    const keywordsArray = Array.isArray(frontmatter.keywords) ? frontmatter.keywords : [];
    if (keywordsArray.length === 0) {
      analysis.suggestions.push('Consider adding relevant keywords for better discoverability');
    } else if (keywordsArray.length > 10) {
      analysis.suggestions.push('Too many keywords. Focus on 5-8 most relevant ones');
    }
    
    // Content length analysis
    if (wordCount < SEOReviewer.WORD_COUNT_MIN) {
      analysis.issues.push(`Content is very short (${wordCount} words). Aim for at least ${SEOReviewer.WORD_COUNT_RECOMMENDED} words`);
    } else if (wordCount < SEOReviewer.WORD_COUNT_RECOMMENDED) {
      analysis.suggestions.push(`Content is on the shorter side (${wordCount} words). Consider expanding`);
    }
    ...
    if (headings.length === 0) {
      ...
      if (h1Count > 1) {
        analysis.issues.push('Multiple H1 tags found. Use only one H1 per page');
      }
      
      const h2Count = headings.filter(h => h.level === 2).length;
      if (h2Count === 0) {
        analysis.suggestions.push('No H2 headings found. Add section headings for better structure');
      }
    }
    ...
    for (const img of images) {
      if (!img.alt || img.alt.trim() === '') {
        if (img.hasNearbyCaption && img.caption) {
          analysis.suggestions.push(`Image missing alt text: ${img.src}. Consider using the caption text: "${img.caption}"`);
        } else {
          analysis.issues.push(`Image missing alt text: ${img.src}`);
        }
      } else if (img.alt.length < SEOReviewer.ALT_TEXT_MIN_LENGTH) {
        if (img.hasNearbyCaption && img.caption && img.caption.length > SEOReviewer.ALT_TEXT_MIN_LENGTH) {
          analysis.suggestions.push(`Alt text too short for image: ${img.src}. Consider using the caption text: "${img.caption}"`);
        } else {
    ...
    if (frontmatter.slug) {
      if (frontmatter.slug.length > SEOReviewer.SLUG_MAX_LENGTH) {
        analysis.suggestions.push('Slug is quite long. Consider shortening for better URLs');
      }
      if (!/^[a-z0-9-]+$/.test(frontmatter.slug)) {
        analysis.suggestions.push('Slug should only contain lowercase letters, numbers, and hyphens');
      }
    }
  }
  ...
  performLLMOptimization(analysis) {
    ...
    if (questionHeadings.length === 0) {
      analysis.suggestions.push('Consider adding question-style headings (What is..., How to...) for better LLM discovery');
    }
    
    // Check for clear problem/solution structure
    const hasProblemSection = content.toLowerCase().includes('problem') || 
                            content.toLowerCase().includes('challenge') ||
                            content.toLowerCase().includes('issue');
    const hasSolutionSection = content.toLowerCase().includes('solution') || 
                             content.toLowerCase().includes('approach') ||
                             content.toLowerCase().includes('answer');
    
    if (!hasProblemSection || !hasSolutionSection) {
      analysis.suggestions.push('Consider adding clear problem/solution structure for better LLM understanding');
    }
    
    // Check for code examples
    const hasCodeBlocks = content.includes('```') || content.includes('<code>');
    if (hasCodeBlocks) {
      analysis.suggestions.push('Great! Code examples help LLMs understand technical content');
    }
    
    // Check for technical keywords
    const techKeywords = ['api', 'sdk', 'library', 'framework', 'database', 'cloud', 'storage', 'performance'];
    const keywordsArray = Array.isArray(frontmatter.keywords) ? frontmatter.keywords : [];
    const hasRelevantTechTerms = techKeywords.some(keyword => 
      content.toLowerCase().includes(keyword) || 
      keywordsArray.some(k => k && k.toLowerCase().includes(keyword))
    );
    
    if (!hasRelevantTechTerms) {
      analysis.suggestions.push('Consider including relevant technical keywords for better categorization');
    }
    
    // Check for context and background
    if (!content.toLowerCase().includes('background') && !content.toLowerCase().includes('context')) {
      analysis.suggestions.push('Consider adding background context for better LLM comprehension');
    }
    ...
    if (!hasConclusion) {
      analysis.suggestions.push('Consider adding a conclusion section to summarize key points');
    }
  }
  ...
  extractContentContext(analysis) {
    ...
    const allText = `${title} ${headings.map(h => h.text).join(' ')} ${content}`.toLowerCase();
    
    const techKeywords = [
      'ai', 'machine learning', 'llm', 'artificial intelligence',
      'object storage', 's3', 'cloud', 'multi-cloud', 'database',
      'api', 'sdk', 'terraform', 'kubernetes', 'docker',
      'python', 'javascript', 'go', 'rust', 'java',
      'performance', 'latency', 'scalability', 'distributed',
      'security', 'encryption', 'authentication', 'replication'
    ];
    
    const conceptKeywords = [
      'development', 'coding', 'programming', 'software',
      'infrastructure', 'architecture', 'deployment', 'migration',
      'optimization', 'automation', 'integration', 'workflow',
      'analytics', 'monitoring', 'backup', 'disaster recovery'
    ];
  ...
  extractMainTopics(allText, techTerms) {
    const mainTopics = [];
    if (techTerms.includes('ai') || allText.includes('artificial intelligence')) mainTopics.push('AI');
    if (allText.includes('development') || allText.includes('coding')) mainTopics.push('development');
    if (allText.includes('software')) mainTopics.push('software');
    if (allText.includes('storage') || allText.includes('data')) mainTopics.push('storage');
    return mainTopics;
  }
  ...
  determineActionWord(allText) {
    if (allText.includes('tutorial') || allText.includes('guide')) return 'Learn how';
    if (allText.includes('compare') || allText.includes('vs')) return 'Discover';
    if (allText.includes('build') || allText.includes('create')) return 'Build';
    if (allText.includes('optimize') || allText.includes('improve')) return 'Optimize';
  ...
  buildDescription(context, actionWord) {
    ...
    if (isTigrisPost) {
      ...
      const primaryTech = techTerms.slice(0, 2).join(' and ') || mainTopics.join(' and ') || 'technology';
  ...
  buildTigrisDescription(context, actionWord) {
    ...
    const tigrisValueProps = ['globally distributed', 'multi-cloud', 'low-latency', 'S3-compatible', 'no egress fees'];
    
    const tigrisContext = allText.match(/tigris[^.]*[.!?]/gi) || [];
    const isTigrisFocused = tigrisContext.length > 2 || title.toLowerCase().includes('tigris');
    
    const isAboutDevTools = (title.toLowerCase().includes('development') && !title.toLowerCase().includes('tigris')) || 
    ...
    if (isAboutDevTools) {
      const devConcept = concepts.find(c => ['development', 'coding', 'programming', 'software'].includes(c)) || 'development';
    ...
    if (isTigrisFocused) {
      const relevantValueProp = tigrisValueProps.find(prop => 
        allText.includes(prop.replace('-', ' ')) || allText.includes(prop.replace('-', ''))
      ) || 'globally distributed';
      
      if (techTerms.includes('ai') || allText.includes('model')) {
        return `${actionWord} AI applications with Tigris ${relevantValueProp} object storage. ${concepts[0] || 'Development'} guide with practical examples.`;
      }
    ...
    if (allText.includes('tool') || allText.includes('ide') || allText.includes('development tools')) {
  ...
  adjustDescriptionLength(description, shouldShorten, isTigrisPost) {
    const targetLength = shouldShorten ? SEOReviewer.DESCRIPTION_TARGET_LENGTH_SHORT : SEOReviewer.DESCRIPTION_TARGET_LENGTH;
    ...
    if (description.length < SEOReviewer.DESCRIPTION_MIN_LENGTH) {
      ...
      if (description.length + additionalContext.length <= SEOReviewer.DESCRIPTION_MAX_LENGTH) {
  ...
  generateImprovedDescription(analysis, shouldShorten = false) {
    ...
    return this.adjustDescriptionLength(description, shouldShorten, context.isTigrisPost);
  }
  ...
  performTigrisTerminologyAnalysis(analysis) {
    ...
    const tigrisFeatures = {
      'object storage': {
        ...
        context: 'Use "object storage" to maintain consistency with S3 terminology'
      },
      'multi-cloud': {
      ...
      's3-compatible': {
        preferred: ['S3-compatible', 'S3 API', 'built-in support for the S3 API'],
    ...
    const deprecatedTerms = [
      { term: 'Amazon S3 compatible', preferred: 'S3-compatible', reason: 'Avoid competitor branding' },
      { term: 'cross-cloud', preferred: 'multi-cloud', reason: 'Standard industry terminology' },
      { term: 'fast access', preferred: 'low-latency access', reason: 'More technical and precise' }
    ];
    ...
    for (const desc of preferredDescriptions) {
      if (fullText.includes(desc.toLowerCase())) {
        ...
        analysis.suggestions.push(`Great! Using preferred Tigris description: "${desc}"`);
        break;
      }
    }
    ...
    if (mentionsTigris && !hasPreferredDescription) {
      analysis.suggestions.push('Consider adding a standard Tigris description. Suggested: "Tigris is a globally distributed, multi-cloud object storage service with built-in support for the S3 API"');
    }
    ...
    for (const deprecated of deprecatedTerms) {
      if (contentLower.includes(deprecated.term.toLowerCase())) {
        analysis.suggestions.push(`Consider replacing "${deprecated.term}" with "${deprecated.preferred}". Reason: ${deprecated.reason}`);
      }
    }
    
    // Check for key feature descriptions
    for (const [feature, info] of Object.entries(tigrisFeatures)) {
      const hasPreferred = info.preferred.some(term => contentLower.includes(term.toLowerCase()));
      const hasAlternative = info.alternatives.some(term => contentLower.includes(term.toLowerCase()));
      
      if (hasAlternative && !hasPreferred) {
        analysis.suggestions.push(`Consider using preferred terminology for ${feature}: ${info.preferred.join(' or ')}. ${info.context}`);
      }
    }
    
    // Check for specific Tigris value propositions in title/description
    const valueProps = [
      'low latency',
      'multi-cloud',
      'globally distributed',
    ...
    const titleDesc = `${frontmatter.title || ''} ${frontmatter.description || ''}`.toLowerCase();
    const hasValueProp = valueProps.some(prop => titleDesc.includes(prop));
    
    if (mentionsTigris && !hasValueProp) {
      analysis.suggestions.push('Consider highlighting a key Tigris value proposition in title/description (e.g., "low latency", "multi-cloud", "globally distributed")');
    }
    ...
    for (const [term, info] of Object.entries(technicalTerms)) {
      if (contentLower.includes(term)) {
        const hasAvoidTerms = info.avoid.some(avoid => contentLower.includes(avoid));
        if (hasAvoidTerms) {
          analysis.suggestions.push(`When using "${term}" (${info.context}), avoid mixing with terms like ${info.avoid.join(', ')}. Use ${info.goodWith.join(', ')} instead for consistency`);
        }
      }
    }
    ...
    if (contentLower.includes('tigris')) {
      ...
      const incorrectCaps = tigrisVariations.filter(variation => variation !== 'Tigris' && variation !== 'TIGRIS');
      
      if (incorrectCaps.length > 0) {
        analysis.suggestions.push('Ensure consistent "Tigris" capitalization (capitalize when used as product name)');
      }
    }
  }
  ...
  performTagAnalysis(analysis) {
    const { frontmatter, content, headings } = analysis;
    const allText = `${frontmatter.title || ''} ${frontmatter.description || ''} ${headings.map(h => h.text).join(' ')} ${content}`.toLowerCase();
    ...
    const categoryTags = ['engineering', 'build with tigris', 'customers', 'updates'];
    const currentCategoryTags = currentTags.filter(tag => categoryTags.includes(tag.toLowerCase()));
    const nonCategoryTags = currentTags.filter(tag => !categoryTags.includes(tag.toLowerCase()));
    
    // Standard tag categories used in the blog
    const tagCategories = {
      // Core technical areas - only suggest if clearly relevant
      'Engineering': ['engineering', 'technical implementation', 'software engineering', 'system design'],
      'AI': ['\\bai\\b', 'artificial intelligence', 'machine learning', 'llm', 'neural network', 'deep learning'],
      'Security': ['security', 'encryption', 'authentication', 'authorization', 'vulnerability'],
      'Performance': ['performance', 'benchmark', 'latency', 'throughput', 'speed optimization'],
      'Scalability': ['scalability', 'horizontal scaling', 'load balancing', 'distributed systems'],
      
      // Technologies
      'Python': ['python', 'pytorch', 'dask', 'jupyter', 'pip'],
      'Kubernetes': ['kubernetes', 'k8s', 'container', 'deployment', 'helm', 'kubectl'],
      'Docker': ['docker', 'dockerfile', 'registry', 'image', 'container registry'],
      'Database': ['database', 'foundationdb', 'sql', 'nosql', 'transactions'],
      'Cloud': ['cloud', 'aws', 'azure', 'gcp', 'multi-cloud'],
      
      // Tigris-specific
      'Object Storage': ['object storage', 'blob storage', 's3', 'bucket', 'data storage'],
      'Migration': ['migration', 'migrate', 'transfer', 'move'],
      'Tigris': ['tigris', 'tigrisfs', 'tigrisdata'],
      
      // Content types
      'Thought Leadership': ['future of', 'evolution', 'industry insight', 'trends', 'opinion', 'perspective', 'transformation'],
      'Build with Tigris': ['tutorial', 'step-by-step', 'how to build', 'getting started', 'complete guide'],
      'Features': ['new feature', 'announcement', 'release', 'introducing', 'now available'],
      'Customers': ['customer story', 'case study', 'success story', 'testimonial'],
      'Development': ['development tools', 'coding', 'programming', 'ide', 'developer experience']
    };
    ...
    for (const [category, keywords] of Object.entries(tagCategories)) {
      ...
      const regexKeywords = keywords.filter(k => k.startsWith('\\b') && k.endsWith('\\b'));
      const plainKeywords = keywords.filter(k => !(k.startsWith('\\b') && k.endsWith('\\b')));
      ...
      // Check for matches
      const hasRegexMatch = regexPatterns.has(category) && regexPatterns.get(category).test(allText);
      const hasPlainMatch = plainKeywords.some(keyword => allText.includes(keyword));
    ...
    if ((allText.includes('guide') || allText.includes('tutorial') || allText.includes('how to')) && 
        allText.includes('tigris') && !currentTags.includes('build with tigris')) {
      suggestedTags.add('Build with Tigris');
    }
    
    // Check for missing fundamental tags
    if (currentTags.length === 0) {
      analysis.issues.push('No tags found. Add relevant tags for better discoverability');
    } else if (currentTags.length < 2) {
      analysis.suggestions.push('Consider adding more tags (2-5 recommended) for better categorization');
    } else if (currentTags.length > 6) {
      analysis.suggestions.push('Too many tags. Focus on 3-5 most relevant ones');
    }
    
    // Suggest missing tags
    if (suggestedTags.size > 0) {
      const suggestions = Array.from(suggestedTags).slice(0, 5); // Limit to 5 suggestions
      analysis.suggestions.push(`Consider adding tags: ${suggestions.join(', ')}`);
    }
    ...
    for (const tag of nonCategoryTags) {
      ...
      if (tag === 'engineering' && !currentTags.includes('Engineering')) {
        inconsistentTags.push('Use "Engineering" instead of "engineering" for consistency');
      }
    }
    
    // Validate category tag positioning
    if (currentCategoryTags.length > 0) {
      const firstCategoryIndex = currentTags.findIndex(tag => categoryTags.includes(tag.toLowerCase()));
      const hasNonCategoryBeforeCategory = currentTags.slice(0, firstCategoryIndex).some(tag => !categoryTags.includes(tag.toLowerCase()));
      
      if (hasNonCategoryBeforeCategory) {
        analysis.suggestions.push('Category tags (Engineering, Build with Tigris, Customers, Updates) should be at the top of the tag list');
      }
    }
    ...
    for (const tag of currentTags) {
      if (deprecatedTags[tag]) {
        analysis.suggestions.push(`Consider replacing "${tag}" with "${deprecatedTags[tag]}" for better consistency`);
      }
    }
  }
  ...
  generatePostReport(analysis) {
    const { filePath, frontmatter, wordCount, headings, images, issues, suggestions } = analysis;
    
    const report = [];
    report.push(`# SEO Review: ${frontmatter.title || path.basename(filePath)}`);
    ...
    if (issues.length === 0 && suggestions.length === 0) {
      ...
      report.push('This post looks great from an SEO and LLM optimization perspective.');
      report.push('');
    }
    ...
    report.push(`- **Title:** ${frontmatter.title ? `"${frontmatter.title}" (${frontmatter.title.length} chars)` : 'Missing'}`);
    report.push(`- **Description:** ${frontmatter.description ? `${frontmatter.description.length} chars` : 'Missing'}`);
    report.push(`- **Keywords:** ${frontmatter.keywords ? frontmatter.keywords.length + ' keywords' : 'None'}`);
    report.push(`- **Tags:** ${frontmatter.tags ? frontmatter.tags.join(', ') : 'None'}`);
    report.push('');
  ...
  generateSummaryReport(analyses) {
    ...
    report.push(`- **Posts with issues:** ${postsWithIssues}/${analyses.length}`);
    ...
    if (postsNeedingAttention.length > 0) {
      ...
      postsNeedingAttention.forEach(analysis => {
        ...
        report.push(`- **${filename}:** ${analysis.issues.length} issues, ${analysis.suggestions.length} suggestions`);
      });
      report.push('');
    }
    ...
    if (bestPosts.length > 0) {
      ...
      bestPosts.forEach(analysis => {
        const filename = path.basename(path.dirname(analysis.filePath));
        report.push(`- **${filename}:** No issues, ${analysis.suggestions.length} minor suggestions`);
      });
      report.push('');
    }
  ...
  async applySuggestions(filePath, analysis) {
    ...
    if (analysis.suggestions.some(s => s.startsWith('Consider adding tags:')) ||
        analysis.suggestions.some(s => s.includes('Use "') && s.includes('for consistency')) ||
        analysis.suggestions.some(s => s.includes('Category tags') && s.includes('should be at the top'))) {
      
      const currentTags = Array.isArray(originalFrontmatter.data.tags) ? originalFrontmatter.data.tags : [];
      let newTagsArray = [...currentTags]; // Start with current tags

      // Add suggested tags
      const suggestedTagsMatch = analysis.suggestions.find(s => s.startsWith('Consider adding tags:'));
      if (suggestedTagsMatch) {
        const tagsToAdd = suggestedTagsMatch.match(/tags: (.*)/)[1].split(', ').map(t => t.trim());
      ...
      // Fix tag casing
      const aiConsistencyMatch = analysis.suggestions.find(s => s.includes('Use "AI" instead of "ai" for consistency'));
      if (aiConsistencyMatch) {
        newTagsArray = newTagsArray.map(tag => tag.toLowerCase() === 'ai' ? 'AI' : tag);
        console.log(`- Corrected "ai" to "AI"`);
      }
      const engineeringConsistencyMatch = analysis.suggestions.find(s => s.includes('Use "Engineering" instead of "engineering" for consistency'));
      if (engineeringConsistencyMatch) {
        newTagsArray = newTagsArray.map(tag => tag.toLowerCase() === 'engineering' ? 'Engineering' : tag);
        console.log(`- Corrected "engineering" to "Engineering"`);
      }

      // Reorder category tags
      const categoryPositioningMatch = analysis.suggestions.find(s => s.includes('Category tags') && s.includes('should be at the top'));
      if (categoryPositioningMatch) {
        const categoryTagsLower = ['engineering', 'build with tigris', 'customers', 'updates'];
        let presentCategoryTags = newTagsArray.filter(tag => categoryTagsLower.includes(tag.toLowerCase()));
        let remainingTags = newTagsArray.filter(tag => !categoryTagsLower.includes(tag.toLowerCase()));
        ...
        presentCategoryTags = presentCategoryTags.map(tag => originalCategoryTagsMap[tag.toLowerCase()] || tag);
        
        updatedFrontmatterData.tags = [...presentCategoryTags, ...remainingTags];
      ...
      const deprecatedTagsMap = { // Only those explicitly suggested for replacement
        'vibe-coding': 'vibe coding',
        'end-to-end-encryption': 'security',
        'open beta': 'features'
      };
      for (const deprecatedTerm in deprecatedTagsMap) {
          if (newTagsArray.includes(deprecatedTerm)) {
              ...
              console.log(`- Replaced deprecated tag "${deprecatedTerm}" with "${deprecatedTagsMap[deprecatedTerm]}"`);
          }
      }
    }
    ...
    for (const deprecated of deprecatedTerms) {
        ...
        if (updatedContentBody.match(regex)) {
            updatedContentBody = updatedContentBody.replace(regex, deprecated.preferred);
            console.log(`- Replaced "${deprecated.term}" with "${deprecated.preferred}" in content.`);
        }
    }
    ...
    const captionSuggestions = analysis.suggestions.filter(s => 
        ...
        (s.includes('Image missing alt text:') || s.includes('Alt text too short for image:'))
    );
    
    for (const suggestion of captionSuggestions) {
        const imagePathMatch = suggestion.match(/Image (?:missing alt text|Alt text too short for image): ([^.]+\.[^.]+)/);
        const captionMatch = suggestion.match(/Consider using the caption text: "([^"]+)"/);
        
        if (imagePathMatch && captionMatch) {
            const imagePath = this.sanitizeImagePath(imagePathMatch[1]);
            const captionText = captionMatch[1].replace(/["'<>]/g, '').substring(0, 200); // Sanitize caption
            ...
            try {
                ...
                if (updatedContentBody.includes(imagePath)) {
                    updatedContentBody = updatedContentBody.replace(markdownImageRegex, `![${captionText}]($2)`);
                    console.log(`- Applied caption as alt text for image: ${imagePath.substring(0, 50)}...`);
                }
                ...
                updatedContentBody = updatedContentBody.replace(htmlImageRegex, (match, attributes) => {
                    if (attributes.includes('alt=')) {
                        // Replace existing alt text
                        return match.replace(/alt=["'][^"']*["']/, `alt="${captionText}"`);
                    } else {
                        // Add alt text
                ...
                console.warn(`- Skipped caption application for ${imagePath}: ${error.message}`);
            }
        }
    }

    // Combine updated frontmatter and content body
    const newFrontmatterString = this.serializeFrontmatter(updatedFrontmatterData);
   ...
   */
  async run(postPathArg = null, applyChanges = false, analyzeAllExplicitly = false) {
    try {
      ...
      if (analyzeAllExplicitly) {
          ...
          if (await this.fileExists(indexMdx)) {
              ...
              console.log('No specific post detected, analyzing all posts by default.');
              targetPaths = (await this.analyzeAllPosts()).map(a => a.filePath);
          }
      }
      ...
      if (targetPaths.length === 0) {
          ...
          console.error('Usage: node seo-reviewer.js [blog-post-path] [--apply] [--all]');
          process.exit(1);
      }
